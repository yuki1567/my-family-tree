...
```bash
# 1. Issue選択とアサイン
## 1-1. オープン状態 & 担当者なし の issue を 100件まで JSON 形式で取得
ISSUE_DATA=$(gh issue list --state open --assignee "" --json number,title,labels --limit 100)

## 1-2. 優先度順に並び替えて対象のissueを選択
ISSUE_NUMBER=$(echo "$ISSUE_DATA" | jq -r '
  map({
    number: .number,
    title: .title,
    priority: (.title | if test("\\[[0-9]+\\]") then
...
ISSUE_LABELS=$(echo "$ISSUE_JSON" | jq -r '.labels | map(.name)')
