...
class Document extends Node_js_1.default {
    ...
    createElementNS(namespaceURI, qualifiedName, options) {
        ...
        if (!qualifiedName) {
            throw new this[PropertySymbol.window].DOMException("Failed to execute 'createElementNS' on 'Document': The qualified name provided is empty.");
        }
        // SVG element
        if (namespaceURI === NamespaceURI_js_1.default.svg) {
            const elementClass = qualifiedName === 'svg'
        ...
        // Custom HTML element
        const customElement = this[PropertySymbol.window].customElements[PropertySymbol.registry]?.[options && options.is ? String(options.is) : qualifiedName];
        if (customElement) {
        ...
        const elementClass = HTMLElementConfig_js_1.default[localName]
            ? this[PropertySymbol.window][HTMLElementConfig_js_1.default[localName].className]
            : null;
        // Known HTML element
        if (elementClass) {
            const element = NodeFactory_js_1.default.createNode(this, elementClass);
